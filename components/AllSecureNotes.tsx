...
export const AllSecureNotes = (
    ...
    const { secureNotes: rawSecureNotes, deleteSecureNote } = useOfflineCacheService()
    ...
    const secureNotes = rawSecureNotes.filter((secureNote) => {
        return secureNote.title.toLowerCase().includes(currentFilterQuery.toLowerCase()) ||
            secureNote.content.toLowerCase().includes(currentFilterQuery.toLowerCase())
    })
